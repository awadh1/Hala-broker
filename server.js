...
async function main() {
    ...
    function tryModel(idx) {
      ...
      fetch('https://generativelanguage.googleapis.com/v1beta/models/' + GEMINI_MODELS[idx] + ':generateContent', {
        ...
        .then(function (res2) {
          ...
          if (res2.status === 429) { console.error('[Gemini] تجاوزت الحصة، ننتقل لـ Groq لو متوفر'); done(false); return; }
          ...
          if (!reply) { console.error('[Gemini] رد غير متوقع:', JSON.stringify(res2.data).slice(0, 300)); done(false); return; }
          done(true, reply);
        }).catch(function (e) { clearTimeout(killer); console.error('[Gemini] خطأ اتصال:', e && e.message); done(false); });
    }
    tryModel(0);
  }
    ...
    function tryModel(idx) {
      ...
      fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        .then(function (res2) {
          ...
          if (!reply) { console.error('[Groq] رد غير متوقع:', JSON.stringify(res2.data).slice(0, 300)); done(false); return; }
          done(true, reply);
        }).catch(function (e) { clearTimeout(killer); console.error('[Groq] خطأ اتصال:', e && e.message); done(false); });
    }
    tryModel(0);
  }
  ...
  function callGemini(systemPrompt, userText, res, logTag) {
    ...
    tryGemini(systemPrompt, userText, function (ok1, reply1) {
      if (ok1) { res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ reply: reply1 })); return; }
      tryGroq(systemPrompt, userText, function (ok2, reply2) {
