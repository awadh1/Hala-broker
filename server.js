    ...
    if (req.method === 'POST' && req.url === '/aqua') {
      ...
      req.on('end', function () {
        ...
        fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent', {
          ...
          body: payload,
          signal: ctrl.signal
        }).then(function (r) { clearTimeout(killer); return r.json(); }).then(function (data) {
          var reply = data && data.candidates && data.candidates[0] && data.candidates[0].content &&
            data.candidates[0].content.parts && data.candidates[0].content.parts[0] && data.candidates[0].content.parts[0].text;
          if (!reply) {
            console.error('[أكوا] رد غير متوقع من Gemini:', JSON.stringify(data).slice(0, 300));
